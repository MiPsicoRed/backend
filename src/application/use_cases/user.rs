...
impl UserUseCases {
    ...
    pub async fn user_onboarded(&self, user_id: &Uuid) -> AppResult<()> {
        ...
        let patient = self.patient_persistence.read_by_user(user_id).await?;
        if let Some(patient_id) = patient.id {
             self.patient_persistence.update_birthdate(&patient_id, birthdate).await?;

             // 2. Check if minor
             let now = chrono::Utc::now().date_naive();
             let age_years = now.years_since(birthdate).unwrap_or(0);
             info!("User birthdate: {:?}, Now: {:?}, Calculated age: {}", birthdate, now, age_years);

             if age_years < 18 {
                 info!("User is a minor. Checking guardian details...");
                 if let (Some(g_name), Some(g_id), Some(sig)) = (guardian_name, guardian_id_document, signature) {
                     info!("Guardian details present. Creating consent record.");
                     // Create Parent Consent
                     let consent = crate::domain::entities::parent_consent::ParentConsent {
                         id: None, // Generated by DB/Persistence
                         patient_id,
                         guardian_name: g_name,
                         guardian_id_document: g_id,
                         signature_data: sig, // TODO: Encrypt this? For now storing base64
                         consent_certificate_id: Uuid::new_v4().to_string(), // Generating a unique ID
                         signed_at: Some(chrono::Utc::now()),
                         created_at: Some(chrono::Utc::now()),
                     };
                     self.parent_consent_persistence.create(&consent).await?;
                     info!("Consent record created successfully.");
                 } else {
                     info!("Missing guardian details for minor.");
                     return Err(crate::app_error::AppError::InvalidPayload); // Missing guardian info
                 }
             } else {
                 info!("User is an adult. Skipping consent creation.");
             }
        } else {
            return Err(crate::app_error::AppError::Internal("Patient ID validation failed".into()));
        }
